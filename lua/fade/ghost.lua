...
local DEMO_NAMESPACE = vim.api.nvim_create_namespace("fade.ghost.demo")

---Multi-token on purpose: a keyword, a call, a string and a number should each stay distinct.
local SAMPLE = table.concat({
  "data = json.loads(raw)",
  "if strict and not isinstance(data, dict):",
  '    raise TypeError(f"expected object, got {type(data)!r}")',
  "return data",
}, "\n")


---Draw a sample suggestion at the cursor, the way a provider would, so toggling and re-running
---compares the flat original against the per-token version. Nothing is written; Python sample.
function M.demo()
  ...
  local virt_lines = {}
  for i = 2, #lines do virt_lines[i - 1] = { { lines[i], "Comment" } } end

  local id = vim.api.nvim_buf_set_extmark(buf, DEMO_NAMESPACE, row - 1, col, {
    virt_text = { { lines[1], "Comment" } },
    virt_text_pos = "inline",
    virt_lines = virt_lines,
  })
  -- A no-op while disabled, which is what leaves the flat color there to compare against.
  M.recolor(buf, DEMO_NAMESPACE, id)

  vim.api.nvim_create_autocmd({ "CursorMoved", "CursorMovedI", "InsertEnter", "BufLeave" }, {
    once = true,
    buffer = buf,
    desc = "Dismiss the ghost text sample",
    callback = function() pcall(vim.api.nvim_buf_clear_namespace, buf, DEMO_NAMESPACE, 0, -1) end,
  })
end
